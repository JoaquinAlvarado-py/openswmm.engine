...
BASE_CASE = ("EPA/m1", "A", "sha256:aaa", "git:1111", "options:cccc")
...
def test_every_case_id_field_changes_the_result():
    ...
    # the build because two executables can print one version string, the
    # dependency because a deck's external data can change under a fixed deck,
    # and the option set because a variant letter means only what
    # `variants.OPTIONS` says it means today.
...
def test_case_id_fields_cannot_be_repartitioned_into_each_other():
    ...
    assert (schema.case_id("a", "b", "c", "d", "e")
            != schema.case_id("ab", "", "c", "d", "e"))
    assert (schema.case_id("a", "b", "c", "d", "e")
            != schema.case_id("", "ab", "c", "d", "e"))
